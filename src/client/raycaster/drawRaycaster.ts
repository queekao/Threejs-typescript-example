...
window.addEventListener("keyup", function (event) {
  if (event.key === "Control") {
    ...
    // console.log("something", drawingLine);
  ...
  if (ctrlDown) {
    ...
    if (intersects.length > 0) {
      if (!drawingLine) {
        ...
        line.frustumCulled = false; // no matter how i move line will not disappear anyway
        scene.add(line);
        // set a label
...
function onDocumentMouseMove(event: MouseEvent) {
  ...
  if (drawingLine) {
    ...
    if (intersects.length > 0) {
      ...
      measurementLabels[lineId].position.lerpVectors(v0, v1, 0.7); // this is where the label will be positioned in
    }
  }
}
