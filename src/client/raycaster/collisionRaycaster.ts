...
// this demo is use camera view distance as raycaster
...
controls.addEventListener("change", function () {
  // controls target is the origin pointer
  ...
  raycaster.set(
    controls.target,
    dir.subVectors(camera.position, controls.target).normalize() // calculate the Vector3 and normalize is for reseting vector
  );
  ...
  if (intersects.length > 0) {
    if (intersects[0].distance < controls.target.distanceTo(camera.position)) {
      //   console.log(
      //     intersects[0].object.setRotationFromMatrix(camera.projectionMatrix),
      //     camera.projectionMatrix
      //   );
      camera.position.copy(intersects[0].point); // set the new camera postion
    }
  }
});
