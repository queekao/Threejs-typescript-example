...
const cube: any = new THREE.Mesh(boxGeometry, material);
cube.position.x = 5;
scene.add(cube);
...
// use vector to draw your geometry
...
// Custom twist geometry
function twist(geometry: any, factor: any) {
...
let geometry2 = new THREE.BoxGeometry(1, 1, 1, 10, 10, 10);
twist(geometry2, Math.PI / 2);
...
gui.add(data, "t", -Math.PI, Math.PI, 0.01).onChange((t) => {
  ...
  geometry2 = new THREE.BoxGeometry(1, 1, 1, 10, 10, 10);
  twist(geometry2, t);
  twistedCube.geometry = geometry2;
});
gui.open();
