...
const myObject3D: any = new THREE.Object3D();
  ...
  Object.keys(clients).forEach((p) => {
    ...
    if (!clientCubes[p]) {
      ...
      if (clients[p].p) {
        ...
        //   clients[p].p.z
        // );
        // use tween make the animation much smooth
...
socket.on("removeClient", (id: string) => {
  // remove which cube should be remove
...
const animate = function () {
  ...
  if (clientCubes[myId]) {
    // camera always lookAt current client ID
